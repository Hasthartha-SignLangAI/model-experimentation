"""
real_time_inference_optionA_with_fewshot_INTERACTIVE_v3.py
...
✅ Fixed-duration recording per sample (default 6s, matches your 5–8s style)
✅ Countdown + GO beep
✅ After capture lockout (prevents double counting)
✅ Optional idle stability requirement before arming

Few-shot improvements:
✅ Supports 'idle' prototype too (blocks outputs when it looks like idle)

Run:
  python real_time_inference_optionA_with_fewshot_INTERACTIVE_v3.py
"""
...
# If few-shot thinks it's "idle" with high similarity => block output
FEWSHOT_IDLE_LABEL = "idle"
FEWSHOT_IDLE_SIM_GATE = 0.80
...
#ENROLL_LOCKOUT_SEC = 1.2
#ENROLL_REQUIRE_IDLE_SEC = 0.8
ENROLL_IDLE_MARGIN = 1.10  # idle-ish if d_s < end_th * margin

ENROLL_COUNTDOWN_SEC = 2        # was 3
ENROLL_REQUIRE_IDLE_SEC = 0.3   # was 0.8
ENROLL_LOCKOUT_SEC = 0.3        # was 1.2
ENROLL_RECORD_SEC = 5.2         # was 6.0  (still >= 512 frames at ~100Hz)


# ✅ Fixed-duration enrollment (matches your 5–8s)
ENROLL_FIXED_DURATION = True
#ENROLL_RECORD_SEC = 6.0    # set 5.0..8.0 as you like

# =====================
# Utils
# =====================
def beep(times=1, gap=0.15):
...
def ask_enroll_details(existing_words):
    print("\n--- Enroll new gesture ---")
    while True:
        w = input("Enter new word name (no spaces) [ex: amma / nil / idle]: ").strip().lower()
        if not w:
    ...
    if ENROLL_FIXED_DURATION:
        print(f"   Each sample will record for {ENROLL_RECORD_SEC:.1f}s (fixed duration)")
    return w, k

def countdown(sec: int, word=None):
    if word == "idle":
        print("  Get ready: keep arm relaxed (idle) for this sample.")
    else:
        print("  Get ready: perform the gesture after GO.")
...
def encode_embed_from_buffer(buffer: deque):
    X = np.array(buffer, dtype=np.float32)
    X = emg_dc_remove(X)
    X = normalize(X)
    X = X[np.newaxis, ...].astype(np.float32)
...
if choice == "2":
    ...
    enroll_count = 0
    print("\nENROLL MODE")
    print(" - Waits for idle stability")
    print(" - Countdown, then records fixed duration per sample\n")
else:
...
while True:
    ...
    # =========================
    # ENROLL MODE (fixed-duration capture)
    # =========================
    if ENROLLING and (not in_gesture):
        ...
        if idle_like:
            if idle_stable_start is None:
                ...
                countdown(ENROLL_COUNTDOWN_SEC, word=ENROLL_WORD)

                # Record fixed duration
                record_until = time.time() + ENROLL_RECORD_SEC
                print(f"  Recording for {ENROLL_RECORD_SEC:.1f}s ...")
                while time.time() < record_until:
                    ln = ser.readline().decode(errors="ignore").strip()
                    if not ln or "t_ms" in ln:
                        continue
                    ps = ln.split(",")
                    if len(ps) != 10:
                        continue
                    try:
                        vv = np.array(ps[1:], dtype=np.float32)
                    except:
                        continue
                    buffer.append(vv)
                    # keep energy buffer moving too (optional)
                    dd = emg_energy_delta(vv, idle_emg_mean)
                    moving_avg(energy_buf, dd)

                # Build embedding from last T frames
                emb = encode_embed_from_buffer(buffer)
                update_fewshot_prototype(ENROLL_WORD, emb)
                enroll_count += 1
                print(f"🧩 ENROLLED '{ENROLL_WORD}' {enroll_count}/{ENROLL_K}")
                beep(1)

                # lockout + re-arm conditions
    ...
    # =========================
    # PREDICTION MODE (Option A state machine)
    # =========================
    if not in_gesture:
    ...
    if accepted_base:
        ...
        # if fewshot says idle strongly, block output
        if top1 == FEWSHOT_IDLE_LABEL and s1 >= FEWSHOT_IDLE_SIM_GATE:
            print(f"🛑 IDLE (few-shot) | sim={s1:.3f} | frames={frames} | end={end_reason}")
        else:
